...
| `.\.venv\Scripts\python.exe main.py train-draft` / `draft` | Stubs | — | — | **VERIFIED**: both `raise NotImplementedError` (`main.py:127-149`, `trainDraft`/`runDraft`) |
| `.\.venv\Scripts\python.exe -m pytest -v` | Runs test suite | none | `.pytest_cache/` | **VERIFIED today**: 4 passed, 1 failed in 0.60s. Failure is `tests/test_moneypuck.py::test_load_game_logs_filters_season_and_keeps_situations` — a known live issue, see `fht-debugging-playbook`. Config quirk verified: pytest reports `configfile: pytest.ini (WARNING: ignoring pytest config in pyproject.toml!)` — `pytest.ini` wins over `pyproject.toml`'s `[tool.pytest.ini_options]`. README's `uv run pytest` is equivalent when `uv` is on PATH |
...
| `data/processed/moneypuck_games_2020.csv` | Reused whenever its mtime is newer than `moneypuck_current.csv`'s mtime (`src/moneypuck.py:80-83`, `loadGameLogs`) — i.e. it survives until you replace the current-season download with a newer one | Delete the file, or just replace `moneypuck_current.csv` with a fresher download (rebuild re-reads the 2.6 GB history file — minutes) |
