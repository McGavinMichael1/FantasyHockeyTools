...
| NHL API calls hang or run slowly | 429 retry loops: `src/nhlAPI.py` sleeps 5s (roster), 15s (player landing), verified lines ~11-13, ~39-41 | `grep -n "429\|sleep" src/nhlAPI.py` | Expected cold-cache load for `pickups`/`api_export.py`: **~32 roster calls only** (the per-player stats fetch was deleted July 2026). Thousand-request per-player landing volume now occurs only in `scripts/build_birthdates.py` / `scripts/build_goalie_seasons.py` one-time cache builds |
